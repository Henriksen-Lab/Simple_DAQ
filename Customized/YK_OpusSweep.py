...
def run_single(sweep,order,name=None):
    ...
    msg = msg.replace('.','p')
    msg = msg.replace('-','m')
    savefile_name = name + '_' + msg + f'.{order}'
...
# Sweep Params
TG_background = -1 # unit: V
BG_background = -1 # unit: V
TG_set =[0,1,2,3,4] # unit: V
BG_set=[0,1,2,3,4] # unit: V
# Scan_per_Set: Num of Opus scans per set of voltages
Scan_per_Set = 2 # Integer
# Sets_per_voltage: Num of loops(background and set) per set of voltages
Sets_per_voltage = 2 # Integer

TG_Sweep_rate = 100E-3 # unit: V/s
TG_Step_size = 500E-3 # unit: V
BG_Sweep_rate = 100E-3 # unit: V/s
BG_Step_size = 500E-3 # unit: V

Save_folder = r'C:\Users\Henriksen Lab\Desktop\IR data\Morgan Hopkins practice\Py_test_OPUS\Sweep_Test'
name = 'test'
order = 0
...
for i in range(len(TG_set)):
    ...
    for j in range(Sets_per_voltage):
        new_order = wet_sweep(TG, BG, TG_Step_size, BG_Step_size, TG_Sweep_rate, BG_Sweep_rate, Scan_per_Set, order)
        new_order = wet_sweep(TG_background, BG_background, TG_Step_size, BG_Step_size, TG_Sweep_rate, BG_Sweep_rate, Scan_per_Set, order)
        order = new_order
