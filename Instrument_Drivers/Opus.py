...
def get_opus_data():
    try:
        ...
        # xpm_file =r'C:\Users\Henriksen Lab\Desktop\Labview\XPM Files\B0T_YK101223.xpm'
        # apt_options = opus.get_param_options('apt')
        # print(apt_options)
        filepath = opus.measure_sample(unload=True) # Perform measurement and unload file from OPUS
        data = read_opus(filepath) # Read OPUS file from measurement
        # plt.plot(data.sm.x, data.sm.y) # Plot single-channel sample spectra
        # plt.legend()
        # plt.show()
        # print(len(data.sm.x))
        return data.sm
    except:
        print('Opus connection fail')
